...
# Define structural and content-related terms
structural_terms = [
    "template", "infobox", "layout", "reference", "formatting", "citation", "sourcecheck", "instructions template"
]

content_terms = [
    "update", "addition", "remove", "edit", "correction", "expand", "accuracy", "content", "discussion", "citations","links", "details"
]
...
structural_docs = find_related_documents(structural_terms, df, vect, X)
content_docs = find_related_documents(content_terms, df, vect, X)

# Create a new column to mark the type of discussion
df['discussion_type'] = 'Other'
df.loc[structural_docs, 'discussion_type'] = 'Structural'
df.loc[content_docs, 'discussion_type'] = 'Content'

# Count the number of documents for each type of discussion in each cluster
discussion_counts = df.groupby(['cluster', 'discussion_type']).size().unstack().fillna(0)

# Plot the results
discussion_counts.plot(kind='bar', stacked=True, figsize=(14, 7))
plt.title('Number of Documents Discussing Structural vs Content Changes by Cluster')
plt.xlabel('Cluster')
