...
custom_stopwords = set(['ect', 'hou', 'com', 'recipient', 'archive', 'wikipedia', 'redirect', 'redirects', 'wikipedia redirects', 'wikipediaredirects'])
...
# df['subtopic'] = df['subtopic'].apply(stem_text)
...
X = vect.fit_transform(df['subtopic'])
...
filename='tfidf_subtopic_features.txt'
...
cluster_texts = df.groupby('cluster')['subtopic'].apply(lambda x: ' '.join(x))
